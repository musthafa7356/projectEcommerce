...
public class OfferServiceImpl implements OfferService {
    ...
    public OfferServiceImpl(OfferRepository offerRepository, ProductService productService, CategoryService categoryService, ProductRepository productRepository) {
    ...
    public Offer save(OfferDto offerDto) {
        ...
        try {
            // Check if a offer with the same name already exists
            List<Offer> existingOffer = offerRepository.findByNameAndEnabledIs(offerDto.getName());
            if (!existingOffer.isEmpty()) {
                throw new DisabledProductExistsException("An Offer with the same name already exists.");
            }
        }catch (Exception e) {
            e.printStackTrace();
            return null;
        ...
        if (offerDto.getOfferType().equals("Product")) {
            ...
            Double salesPrice = Double.parseDouble(formattedSalesPrice);
            product.setSalePrice(salesPrice);
